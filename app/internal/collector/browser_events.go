...
func BuildBrowserCrawlEvents(result BrowserCrawlResult, baseLabels map[string]string) []BrowserCrawlEvent {
	...
	for _, page := range result.Pages {
		...
		events = append(events, BrowserCrawlEvent{
			...
			Target:    nonEmptyString(page.FinalURL, page.URL),
			Severity:  "info",
			Message:   fmt.Sprintf("Browser crawl completed for %s with %d script(s)", nonEmptyString(page.FinalURL, page.URL), len(page.Scripts)),
			Labels:    pageLabels,
			Payload:   pagePayload,
		})
		...
		for _, script := range page.Scripts {
			...
			payload := map[string]any{
				...
				"url":                script.URL,
				"url_redacted":       script.URLRedacted,
			...
			events = append(events, BrowserCrawlEvent{
				...
				Target:    scriptTarget,
				Severity:  "info",
				Message:   fmt.Sprintf("Browser observed %s script on %s", nonEmptyString(script.SourceType, "unknown"), nonEmptyString(page.FinalURL, page.URL)),
				Labels:    scriptLabels,
				Payload:   payload,
			})
			if script.TagManager {
				events = append(events, BrowserCrawlEvent{
					...
					Target:    scriptTarget,
					Severity:  "low",
					Message:   fmt.Sprintf("Browser crawl detected tag manager on %s", nonEmptyString(page.FinalURL, page.URL)),
					Labels:    scriptLabels,
					Payload:   payload,
				})
			}
