...
func TestHubRulesEvaluateRunsBuiltInFixtures(t *testing.T) {
	...
	if !strings.Contains(stdout, "Rule fixture evaluation: 8 passed, 0 failed") {
		t.Fatalf("stdout = %q, want passing fixture summary", stdout)
	}
	if !strings.Contains(stdout, "deploy-window-browser-drift") || !strings.Contains(stdout, "multi-host-file-drift") {
