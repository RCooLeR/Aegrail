	...
	_, err = findings.SaveHubFindings(ctx, []domain.HubFinding{
		{
			...
			Title:          "WordPress administrator added",
			Summary:        "User roman@example.test became an administrator.",
			EventIDs:       []domain.ID{"event-1", "event-2"},
			FirstEventAt:   now.Add(-time.Minute),
			LastEventAt:    now.Add(time.Minute),
		},
	})
	if err != nil {
		t.Fatalf("second SaveHubFindings returned error: %v", err)
	}

	third, err := hub.AnalyzeModelAnalysisQueue(ctx, AnalyzeModelAnalysisQueueInput{
		Limit:       10,
		GeneratedAt: now.Add(time.Minute),
	})
	if err != nil {
		t.Fatalf("third AnalyzeModelAnalysisQueue returned error: %v", err)
	}
	if third.Generated != 0 || third.Skipped != 1 || len(model.GenerateRequests) != 1 {
		t.Fatalf("third queue result = %#v requests=%d, want existing current-prompt report skipped despite finding update", third, len(model.GenerateRequests))
	}
}
...
func (r *memoryModelAnalysisReportRepository) ListModelAnalysisReportsForFinding(_ context.Context, environmentID domain.ID, appID domain.ID, findingID domain.ID, limit int) ([]domain.ModelAnalysisReport, error) {
	var reports []domain.ModelAnalysisReport
	for _, report := range r.reports {
		if report.EnvironmentID != environmentID {
			continue
...
func (r *memoryModelAnalysisReportRepository) FindLatestModelAnalysisReportByFinding(_ context.Context, environmentID domain.ID, appID domain.ID, findingID domain.ID, promptTemplateID string, promptTemplateVersion string, modelName string) (domain.ModelAnalysisReport, bool, error) {
	var latest domain.ModelAnalysisReport
	for _, report := range r.reports {
		if report.EnvironmentID != environmentID ||
			report.PromptTemplateID != promptTemplateID ||
			report.PromptTemplateVersion != promptTemplateVersion {
			continue
		}
		if appID != "" && report.AppID != appID {
			continue
		}
		if strings.TrimSpace(modelName) != "" && report.ModelName != modelName {
			continue
		}
		hasFinding := false
		for _, sourceFindingID := range report.SourceFindingIDs {
		...
		if latest.ID == "" ||
			report.GeneratedAt.After(latest.GeneratedAt) ||
			(report.GeneratedAt.Equal(latest.GeneratedAt) && report.CreatedAt.After(latest.CreatedAt)) {
			latest = report
		}
	}
	if latest.ID == "" {
