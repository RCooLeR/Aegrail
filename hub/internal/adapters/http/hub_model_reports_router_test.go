...
func TestHubRouterReturnsFindingReviewReport(t *testing.T) {
	inventory := newHTTPTestInventoryRepository()
	findings := newHTTPTestFindingRepository()
	reports := newHTTPTestModelAnalysisReportRepository()
	router := NewHubRouter(domain.AppMeta{Name: "Aegrail", Binary: "aegrail-hub", Version: "test"}, hubapp.New(hubapp.Dependencies{
		Meta:         domain.AppMeta{Name: "Aegrail", Binary: "aegrail-hub", Version: "test"},
		Inventory:    inventory,
		Findings:     findings,
		ModelReports: reports,
	}), HubOptions{})

	request := httptest.NewRequest(http.MethodGet, "/api/v1/reports/finding-review?org=acme&project=customer-site&environment=production&app=main-web", nil)
	response := httptest.NewRecorder()
	router.ServeHTTP(response, request)
	...
	if body.Report.FindingCount != 1 || body.Report.Items[0].Finding.ID != "finding-1" || body.Report.Items[0].ModelReport == nil {
		t.Fatalf("body = %#v, want finding with paired model report", body)
	}
}
...
func (r *httpTestModelAnalysisReportRepository) FindLatestModelAnalysisReportByFinding(ctx context.Context, environmentID domain.ID, appID domain.ID, findingID domain.ID, promptTemplateID string, promptTemplateVersion string, modelName string) (domain.ModelAnalysisReport, bool, error) {
