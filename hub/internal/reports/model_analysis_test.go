...
func TestFormatModelAnalysisResponseUsesStructuredFormat(t *testing.T) {
  ...
  "operator_comment": "This looks risky because a writable-path PHP file appeared near privileged activity. Confirm whether this was part of a planned deployment before marking it safe.",
  "incident_chain": [
    {
  ...
  "uncertainty_and_gaps": [
	...
	requireContains(t, report, "<h4>Operator Comment</h4>")
	requireContains(t, report, "planned deployment")
...
func TestBuildModelAnalysisPromptProfilesWebAccessLogs(t *testing.T) {
	...
	prompt, err := BuildModelAnalysisPrompt(bundle, ModelAnalysisOptions{
		AppKind:           "wordpress",
		FindingRuleID:     "web-admin-success-after-failures",
		FindingID:         "finding-1",
		FindingTitle:      "Admin request succeeded after failures",
		FindingSummary:    "Admin POST succeeded after multiple failures.",
		FindingSeverity:   "high",
	...
	requireContains(t, prompt.User, "Issue type: web_access_activity")
...
func TestBuildModelAnalysisPromptProfilesPrestaShopModuleFiles(t *testing.T) {
	...
	prompt, err := BuildModelAnalysisPrompt(bundle, ModelAnalysisOptions{
		AppKind:           "prestashop",
		FindingRuleID:     "file-php-in-writable-path",
		FindingID:         "finding-1",
		FindingTitle:      "PHP file under module asset path",
		FindingSummary:    "A PHP index file was observed below a module image directory.",
	...
	requireContains(t, prompt.User, "Application platform: PrestaShop")
	requireContains(t, prompt.User, "Issue type: file_system_change")
	requireContains(t, prompt.User, "vendor index.php redirect guards")
	requireContains(t, prompt.User, "directory guard or redirect files")
}

func TestInferModelAnalysisIssueTypeCoversKnownFamilies(t *testing.T) {
	tests := map[string]string{
		"probable-incident-chain":                  "incident_chain",
		"web-to-file-change":                       "incident_chain",
		"web-admin-success-after-failures":         "web_access_activity",
		"browser-script-domain-new":                "browser_activity",
		"wordpress-admin-user-added":               "identity_and_access",
		"wordpress-cron-task-added":                "scheduled_task_change",
		"wordpress-active-plugin-added":            "platform_extension_change",
		"prestashop-payment-configuration-changed": "platform_configuration_change",
		"mautic-plugin-upgraded":                   "platform_extension_change",
		"mautic-integration-credentials-changed":   "platform_configuration_change",
		"mautic-oauth-client-changed":              "identity_and_access",
		"database-snapshot-check-changed":          "database_activity",
		"file-php-in-writable-path":                "file_system_change",
	}
	for ruleID, want := range tests {
		if got := inferModelAnalysisIssueType(ruleID); got != want {
