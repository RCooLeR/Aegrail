...
import (
	...
	"github.com/rcooler/aegrail/agent/internal/collector"
		...
		Action: func(c *urfavecli.Context) error {
			...
			runOnce := func(config agent.ServerConfig) error {
				...
				if bootstrap {
					...
					send, err := runtime.SendQueued(ctx, sendLimit)
					if err != nil {
						return err
					}
				...
				printAgentRunCollectorStatus(c.App.Writer, config, result, databaseResult, browserResult, coverageResult, bootstrap)
				if bootstrap {
...
func printAgentRunCollectorStatus(writer io.Writer, config agent.ServerConfig, result agent.ServerRunResult, databaseResult agentDatabaseRunResult, browserResult agentBrowserRunResult, coverageResult agent.ServerConfigCoverageRunResult, bootstrap bool) {
	if len(config.Sites) == 0 {
		return
	}
	fileSites := map[string]agent.ServerRunSiteResult{}
	for _, site := range result.Sites {
		fileSites[site.Slug] = site
	}
	databaseSites := map[string]agentDatabaseSiteRunResult{}
	for _, site := range databaseResult.Sites {
		databaseSites[site.Slug] = site
	}
	browserSites := map[string]agentBrowserSiteRunResult{}
	for _, site := range browserResult.Sites {
		browserSites[site.Slug] = site
	}

	fmt.Fprintln(writer, "Collector status:")
	for _, site := range config.Sites {
		fileSite := fileSites[site.Slug]
		fmt.Fprintf(
			writer,
			"  %s files=%s logs=%s db=%s browser=%s config=%s\n",
			site.Slug,
			agentRunFileStatus(site, fileSite),
			agentRunLogStatus(site, fileSite),
			agentRunDatabaseStatus(site, databaseSites[site.Slug]),
			agentRunBrowserStatus(site, browserSites[site.Slug]),
			agentRunCoverageStatus(site, coverageResult, bootstrap),
		)
	}
}

func agentRunFileStatus(site agent.ServerSiteConfig, result agent.ServerRunSiteResult) string {
	if !cliSiteFilesEnabled(site) {
		return "disabled"
	}
	...
	return collectorRunStatus(result.FileBaselined, result.FilesWatched, result.FilesQueued)
}

func agentRunLogStatus(site agent.ServerSiteConfig, result agent.ServerRunSiteResult) string {
	if len(site.Logs) == 0 {
		return "disabled"
	}
	if result.Slug == "" {
	...
	return collectorRunStatus(result.LogBaselined, result.LogsWatched, result.LogsQueued)
}

func collectorRunStatus(baselined bool, watched int, queued int) string {
	switch {
	case baselined:
		return fmt.Sprintf("baseline(%d)", watched)
	case queued > 0:
		return fmt.Sprintf("queued(%d)", queued)
	default:
		return fmt.Sprintf("ok(%d)", watched)
	}
}

func agentRunDatabaseStatus(site agent.ServerSiteConfig, result agentDatabaseSiteRunResult) string {
	...
	case result.Warnings > 0:
		return fmt.Sprintf("warning(%d)", result.Warnings)
	case result.Skipped > 0 && result.Skipped >= result.Databases:
		return "skipped(schedule)"
	case result.Baselines > 0:
		return fmt.Sprintf("baseline(%d)", result.Baselines)
	case result.Changes > 0:
		return fmt.Sprintf("changes(%d)", result.Changes)
	default:
		return fmt.Sprintf("ok(%d)", result.Databases)
	}
}

func agentRunBrowserStatus(site agent.ServerSiteConfig, result agentBrowserSiteRunResult) string {
	...
	return fmt.Sprintf("ok(%d)", result.Pages)
}

func agentRunCoverageStatus(site agent.ServerSiteConfig, result agent.ServerConfigCoverageRunResult, bootstrap bool) string {
	if !cliSiteCoverageEnabled(site) {
		return "disabled"
	}
	if bootstrap {
...
func cliSiteFilesEnabled(site agent.ServerSiteConfig) bool {
	return site.Files.Enabled == nil || *site.Files.Enabled
}

func cliSiteCoverageEnabled(site agent.ServerSiteConfig) bool {
...
func runConfiguredBrowserCrawls(ctx context.Context, runtime *agent.Runtime, config agent.ServerConfig, bootstrap bool) (agentBrowserRunResult, error) {
	...
	for _, site := range config.Sites {
		...
		crawlResult, err := collectorRuntime.CrawlBrowserPages(ctx, collector.BrowserCrawlInput{
			...
			FallbackUserAgents: crawl.FallbackUserAgents,
			SameHostOnly:       true,
...
func agentSendCommand(name string, usage string) *urfavecli.Command {
	return &urfavecli.Command{
		...
		Action: func(c *urfavecli.Context) error {
			runtime := newAgentRuntime(c)
			result, err := runtime.SendQueued(c.Context, c.Int("limit"))
			if err != nil {
				return err
			}
...
func agentStartCommand() *urfavecli.Command {
	return &urfavecli.Command{
		...
		Flags: append(agentConfigFlags(),
			...
			&urfavecli.StringSliceFlag{Name: "profile", Usage: "watch profile: wordpress, prestashop, mautic, yii2-rbac, laravel"},
		...
		Action: func(c *urfavecli.Context) error {
			...
			runOnce := func() error {
				...
				send, err := runtime.SendQueued(ctx, c.Int("send-limit"))
				if err != nil {
					if c.Bool("once") {
						return err
					}
