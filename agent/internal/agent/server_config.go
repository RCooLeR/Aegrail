...
type ServerHubConfig struct {
	...
	HubPublicKey  string `yaml:"hub_public_key"`
	NodeSecret    string `yaml:"node_secret"`
	NodeSecretEnv string `yaml:"node_secret_env"`
...
type ServerBrowserCrawlConfig struct {
	...
	FallbackUserAgents []string `yaml:"fallback_user_agents"`
...
func NormalizeServerConfig(config ServerConfig) ServerConfig {
	...
	config.Hub.Protocol = strings.TrimSpace(config.Hub.Protocol)
	if config.Hub.Protocol == "" {
		config.Hub.Protocol = "aegrail-wire-v1"
	}
	config.Hub.HubPublicKey = strings.TrimSpace(config.Hub.HubPublicKey)
	config.Hub.NodeSecret = strings.TrimSpace(config.Hub.NodeSecret)
	config.Hub.NodeSecretEnv = strings.TrimSpace(config.Hub.NodeSecretEnv)
	...
	for index := range config.Sites {
		...
		if siteFilesEnabled(*site) && len(site.Files.Profiles) == 0 {
			...
			case "mautic":
				site.Files.Profiles = []string{"mautic"}
			case "yii2-rbac":
...
func ValidateServerConfig(config ServerConfig) error {
	...
	if config.Hub.Protocol != "aegrail-wire-v1" {
		issues = append(issues, "hub.protocol must be aegrail-wire-v1")
	}
	if config.Hub.HubPublicKey == "" {
		issues = append(issues, "hub.hub_public_key is required")
	}
	if ResolveServerConfigNodeSecret(config) == "" {
		issues = append(issues, "hub.node_secret or hub.node_secret_env is required")
	}
...
func (config ServerConfig) AgentIdentity() Identity {
	return Identity{
		...
		HubProtocol:  config.Hub.Protocol,
		HubPublicKey: config.Hub.HubPublicKey,
		NodeSecret:   ResolveServerConfigNodeSecret(config),
...
func ResolveServerConfigNodeSecret(config ServerConfig) string {
	if strings.TrimSpace(config.Hub.NodeSecret) != "" {
		return strings.TrimSpace(config.Hub.NodeSecret)
	}
	if strings.TrimSpace(config.Hub.NodeSecretEnv) == "" {
		return ""
	}
	return strings.TrimSpace(os.Getenv(config.Hub.NodeSecretEnv))
}

func (r *Runtime) RunServerConfigOnce(ctx context.Context, config ServerConfig, sendLimit int, bootstrap bool) (ServerRunResult, error) {
	config = NormalizeServerConfig(config)
	if err := ValidateServerConfig(config); err != nil {
	...
	for _, site := range config.Sites {
		...
		if siteFilesEnabled(site) && (len(site.Files.Profiles) > 0 || len(site.Files.ExtraPaths) > 0) {
			...
			siteResult.FilesQueued = watchResult.Queued
		...
		if len(site.Logs) > 0 {
			...
			siteResult.LogsQueued = logResult.Queued
	...
	if bootstrap {
		...
		sendResult, err := r.SendQueued(ctx, sendLimit)
		if err != nil {
			return result, err
		}
