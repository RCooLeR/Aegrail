...
func TestBuildBrowserCrawlEventsEmitsPageScriptTagManagerAndWarnings(t *testing.T) {
		...
		Pages: []BrowserPageResult{
			{
				...
				FinalURL:   "https://example.test/?token=secret",
				...
				Scripts: []BrowserScriptObservation{
					{
						...
						Path:              "/gtm.js",
						Attributes:        map[string]string{"src": "https://www.googletagmanager.com/gtm.js?id=GTM-ABC123&token=secret"},
						ResponseStatus:    200,
	...
	if events[0].Payload["page_url"] == "https://example.test/?token=secret" || strings.Contains(events[1].Payload["attributes"].(map[string]string)["src"], "secret") {
		t.Fatalf("browser event payload leaked secret material: %#v %#v", events[0].Payload, events[1].Payload)
	}
	if events[0].Payload["user_agent"] != AegrailCrawlerUserAgent {
		t.Fatalf("user_agent payload = %#v", events[0].Payload["user_agent"])
	}
}
