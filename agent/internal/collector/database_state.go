...
func DiffDatabaseSnapshotState(previous DatabaseSnapshotState, found bool, current DatabaseSnapshotState) DatabaseSnapshotDiffResult {
	if !found {
		return DatabaseSnapshotDiffResult{Baselined: true, CurrentUpdatedAt: current.UpdatedAt}
	}
...
func databaseEntityEquivalentForDiff(previous DatabaseEntityState, current DatabaseEntityState) bool {
	...
	if !previous.SourceCreatedAt.Equal(current.SourceCreatedAt) || !previous.SourceUpdatedAt.Equal(current.SourceUpdatedAt) {
		return false
	}
...
func isDatabasePIIAccountEntity(entityType string) bool {
	...
	case "wordpress_user", "prestashop_employee", "mautic_user", "yii2_rbac_user", "laravel_user":
		return true
	default:
		return false
	}
