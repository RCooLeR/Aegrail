# Agent And Evidence Internals

This document explains what the agent actually does under the hood: schedules, collectors, SQL queries, transformations, queue files, signing, and Hub ingest.
...
- Logs: access logs and PHP errors. The agent sends normalized request/error events with secret-bearing query values, cookies, authorization-like strings, tokens, and passwords redacted.
- Databases: read-only WordPress and PrestaShop snapshots. The agent sends counts, hashes/digests of selected option/config values, and entity observations such as users, employees, plugins, themes, modules, cron entries, content-script indicators, and selected configuration keys.
- Browser crawls: rendered or non-rendered page observations. The agent sends script URLs/domains, redacted URL variants, inline script hashes/byte sizes, tag manager IDs, page/final URLs, site icon candidates, and crawl warnings.
...
- WordPress users and PrestaShop employees may include the normalized full email/login in event payloads so findings can identify which account changed.
...
- Raw WordPress option values and PrestaShop configuration values are not emitted. The collector sends counts, SHA-256 digests, byte sizes, selected safe keys, and change signatures.

## Scheduler

`go run ./cmd/aegrail-agent agent run --config <file>` loads the YAML config, normalizes defaults, and repeats `runOnce` every `runtime.interval`. The default interval is `30s` when the config does not set one. The compatibility `cmd/aegrail` binary still exposes the same `agent` commands for older scripts.
...
6. Send pending batches to Hub if an ingest secret is available.
...
- File scans run each loop, but unchanged files are cheap: the agent reuses the previous SHA-256 when path, size, and mtime are unchanged. A forced full hash refresh happens every `1h` by default; override with `AEGRAIL_WATCH_FULL_HASH_INTERVAL`.

## File Collector

For each site with file monitoring enabled, the agent builds a path list from the site profile (`wordpress`, `prestashop`, etc.), `files.extra_paths`, and `files.exclude`.
...
When sending, the agent posts the exact JSON body to:
...
X-Aegrail-Timestamp: <UTC RFC3339 timestamp>
X-Aegrail-Signature: sha256=<HMAC-SHA256 hex>
```

Signature input:

```text
HMAC-SHA256(AEGRAIL_HUB_INGEST_SECRET, timestamp + "\n" + raw_json_body)
```

This is signing, not encryption. It proves the batch came from someone with the shared secret and was not modified in transit. Payload confidentiality comes from HTTPS/private network transport.

## Hub Ingest And PostgreSQL

Hub receives `/api/v1/ingest/events`, limits the body to `2 MiB`, verifies:

```text
timestamp exists
timestamp is inside the accepted skew window
signature exists
signature equals HMAC-SHA256(secret, timestamp + "\n" + raw body)
...
```powershell
go run ./cmd/aegrail-agent agent run --config configs/agent.multi-site.example.yaml --once --bootstrap --discard-pending
```
