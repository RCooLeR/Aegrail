...
- Log tailing for access logs, PHP errors, generic logs, and Tor exit metadata when enabled, with routine public/API/static traffic filtered before it reaches Hub.
- WordPress, WordPress multisite, PrestaShop, Mautic, Yii2 RBAC, and Laravel database snapshots.
- Static, React, and Node.js file profiles with log/browser/config coverage and no default database snapshot profile.
...
- Browser crawl observations for script URLs, domains, inline hashes, redacted inline previews, tag-manager IDs, and favicon candidates.
...
- Persistent database connection pools by default, with coverage visibility when a site is configured for one-shot database connections.
...
- File-event framework/deploy evidence for WordPress, PrestaShop, Mautic, Yii2 RBAC, Laravel, static, React, Node.js, writable PHP, dependency manifests, routes, migrations, views, and source areas.
